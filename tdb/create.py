...
def convert_buffer(vcf, samples, stats, num_lines):
    """
    Converts upto num_lines number of vcf entries
    """
    buffer = {'locus':[],
              'allele':[],
    ...
    while num_lines > 0:
        ...
        buffer['locus'].append(cur_locus)
        buffer['allele'].extend(cur_allele)
        num_samples = 0
        for name, rows in cur_sample.items():
            num_samples += len(rows)
            buffer['sample'][name].extend(rows)
...
def write_tables(cur_tables, tables):
    ...
    ldf = pd.DataFrame(cur_tables["locus"], columns=L_COLUMNS)
    ...
    adf = pd.DataFrame(cur_tables["allele"], columns=A_COLUMNS)
    ...
    for name, out_samp in tables["sample"].items():
        sdf = pd.DataFrame(cur_tables["sample"][name], columns=S_COLUMNS)
...
def create_main(args):
    ...
    parser.add_argument("-b", "--buffer", metavar="BUF", type=int,
                        help="Number of vcf lines to buffer before write (off)")
    ...
    while True:
        cur_tables, cvt_any = convert_buffer(vcf, samples, stats, args.buffer)
        if not cvt_any:
            break
